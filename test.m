...
%% Initial Conditions
    ...
    upsilon{2} = [0.1;0.2;0.3];
    W_c{2} = 200*rand(n_NN,1);
    W_a{2} = 200*rand(n_NN,1);
...
%% H matrix
    ...
    %H = inv(H_inv);
    %dH = H_inv*dH_inv*H;
    R = [cos(q{i}(3)) -sin(q{i}(3)) 0;sin(q{i}(3)) cos(q{i}(3)) 0;0 0 1];
...
%% Update NN Weight
    ...
    delta = -((x{i}(1)^2 + x{i}(2)^2 +x{i}(3)^2 + x{i}(4)^2 + x{i}(5)^2 + x{i}(6)^2)*norm(g_x))^2 + z{i}'*[Q zeros(6); zeros(6) zeros(6)]*z{i} + 1/4*W_a{i}'*D_1*W_a{i} + W_c{i}'*(sigma);
